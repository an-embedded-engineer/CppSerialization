...
namespace cpp_lib
{
    namespace exception
    {
        ...
        {
            /* シンボル情報を取得 */
            const cpp_lib::exception::SymbolInfo symbol_info = SymbolInfoGenerator::GenerateSymbolInfo(raw_symbol_info);
            ...
            if (symbol_info.is_valid == true)
            {
                ...
                if (mangled_symbol_name[0] == '_')
                {
                    ...
                    if (demangled_symbol_name != nullptr)
                    {
                        ...
                        /* シンボル情報テキストをセット */
                        symbol_info_text = cpp_lib::string::Format("%s %s + %s", symbol_info.object_name, function_name, symbol_info.offset);
                    }
                    else
                    {
                        /* シンボル情報テキストをセット */
                        symbol_info_text = cpp_lib::string::Format("%s %s + %s", symbol_info.object_name, mangled_symbol_name, symbol_info.offset);
                    }
                }
                else
                {
        ...
        /* シンボル情報生成 */
        cpp_lib::exception::SymbolInfo SymbolInfoGenerator::GenerateSymbolInfo(const std::string& raw_symbol_info)
        {
            ...
            /* シンボル情報生成 */
            cpp_lib::exception::SymbolInfo symbol_info;
