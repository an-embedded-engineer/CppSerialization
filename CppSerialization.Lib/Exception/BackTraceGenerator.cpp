...
namespace cpp_lib
{
    ...
    {
        /* バックトレース&シンボルリスト生成 */
        void BackTraceGenerator::GenerateBackTraceAndSymbols(size_t& trace_size, std::vector<cpp_lib::exception::address_t>& traces, std::vector<std::string>& symbols)
        {
            /* 最大トレースサイズ */
            constexpr size_t MaxSize = 256;

            /* トレースリスト */
            cpp_lib::exception::address_t raw_traces[MaxSize] = {};
            ...
            trace_size = 0;
            /* 出力用トレースアドレスリストセット  */
            traces = std::vector<cpp_lib::exception::address_t>();
            /* 出力用シンボルリストセット  */
            symbols = std::vector<std::string>();
#endif
        }
        ...
        /* バックトレース生成 */
        void BackTraceGenerator::GenerateBackTrace(size_t& trace_size, std::vector<cpp_lib::exception::address_t>& traces)
        {
            /* 最大トレースサイズ */
            constexpr size_t MaxSize = 256;
        ...
        /* シンボルリスト生成 */
        void BackTraceGenerator::GenerateBackTraceSymbols(const size_t& trace_size, const std::vector<cpp_lib::exception::address_t>& traces, std::vector<std::string>& symbols)
        {
#if PLATFORM_TYPE == PLATFORM_MSVC
            /* 現在のプロセスを取得 */
            HANDLE process = GetCurrentProcess();
