...
namespace cpp_lib
{
    namespace exception
    {
        ...
        const std::string FatalException::GenerateErrorMessage()
        {
            ...
            if (this->m_IsErrorInfoExists == true)
            {
                ss << cpp_lib::string::Format("[Fatal Error] %s @ %s[%s:L.%d]", this->m_Message, this->m_FunctionName, this->m_FilePath, this->m_LineNumber);
            }
            else
            {
                ss << cpp_lib::string::Format("[Fatal Error] %s", this->m_Message);
            }
            ss << std::endl;
